...
const testComponent = sampleSchemas.github.components[0] // GlobalHeader
...
const verificationTests = [
  {
    name: "Fixed Header",
    test: () => {
      const header = sampleSchemas.github.components[0]
  ...
  },
  {
    name: "Sticky Sidebar",
    test: () => {
      const sidebar = sampleSchemas.github.components[1]
  ...
  },
  {
    name: "Flex Layout",
    test: () => {
      const navbar = sampleSchemas.dashboard.components[0]
  ...
  },
  {
    name: "Grid Layout",
    test: () => {
      const cardGrid = sampleSchemas.cardGallery.components[1]
  ...
  },
  {
    ...
    test: () => {
      const main = sampleSchemas.github.components[2]
...
expectedPatterns.forEach(({ schema, component, patterns }) => {
  const comp = (sampleSchemas as any)[schema].components[component]
