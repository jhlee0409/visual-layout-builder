...
/**
 * Prompt Template Types for Schema V2
 *
 * V1과 동일한 구조지만 V2 스키마 특성 반영:
...
export interface PromptTemplateV2 {
  ...
  layoutSection: (
    ...
    layouts: LaydlerSchemaV2["layouts"]
  ) => string
  instructionsSection: () => string
}

/**
 * React + Tailwind CSS Template for Schema V2
 *
 * V2 Component Independence 아키텍처 기반
 */
export const reactTailwindTemplateV2: PromptTemplateV2 = {
  framework: "react",
  cssSolution: "tailwind",

  systemPrompt: `You are an expert React developer. Generate a responsive layout component based on the following Schema V2 specifications.
...
- Apply mobile-first responsive design: base styles for mobile, then md: for tablet, lg: for desktop`,
  ...
  layoutSection: (breakpoints: Breakpoint[], layouts: LaydlerSchemaV2["layouts"]) => {
...
/**
 * Template registry for V2 - extensible for future frameworks
 */
export const templateRegistryV2: Record<
  string,
  Record<string, PromptTemplateV2>
> = {
  react: {
    tailwind: reactTailwindTemplateV2,
...
/**
 * Get template by framework and CSS solution (V2)
 */
export function getTemplateV2(
  framework: string,
  cssSolution: string
): PromptTemplateV2 | null {
  return templateRegistryV2[framework]?.[cssSolution] || null
}
