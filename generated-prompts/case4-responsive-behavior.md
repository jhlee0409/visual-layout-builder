...
### 1. ResponsiveNav (c1)
- **Semantic Tag:** `<nav>`
- **Component Name:** `ResponsiveNav`

**Positioning:**
- Type: `sticky`
- Position values: top: 0, zIndex: 40
...
**Responsive Behavior:**
- Mobile: hidden, order: 1
- Tablet (md:): visible, width: 50%, order: 2
- Desktop (lg:): width: 33.333%, order: 3
...
- Row 0: ResponsiveNav (c1, full width)

**Spatial Relationships:**

- **ResponsiveNav (c1)** spans **FULL WIDTH** as a full-width section
...
Components:
- **ResponsiveNav (c1)**: `col-span-full row-span-1`
...
This breakpoint uses a **8-column × 8-row grid system** with 1 components.
...
Container: `grid grid-cols-8 grid-rows-8 gap-4`
...
- Each component MUST use `grid-area` (or `grid-column`/`grid-row`) to specify its exact position based on Canvas Grid coordinates
- Each component still uses its own `positioning` strategy (sticky/fixed/static) and internal `layout` (flex/grid/container)
- This grid layout applies to the **tablet** breakpoint - other breakpoints may have different arrangements
